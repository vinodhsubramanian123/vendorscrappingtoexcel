...
function updateScrapedRegistry(info) {
  ...
  const pdfStr  = info.pdfPath ? `[PDF](${info.pdfPath})` : 'Advisory (No QS Link)';

  const newRow =
    `| ${dateStr} | ${info.solutionName || 'OCA Solution'} | ${info.family} | ` +
    `${info.gen} | \`${info.chassisName}\` | **${info.skuCount || info.tablesCount}** | ` +
    `[${path.basename(info.xlsxPath)}](${info.xlsxPath}) | [${path.basename(info.jsonPath)}](${info.jsonPath}) | ` +
    `${pdfStr} | \`${info.outputDir.replace(/.*\/outputs\//, 'outputs/')}/\` |\n`;

  // Dedup by full output path
  if (!content.includes(info.outputDir)) {
    // Insert after table header divider line if table exists
    const divider = '| :--- | :--- | :--- | :--- | :--- | ---: | :--- | :--- | :--- | :--- |\n';
    ...
    console.log(`Registry already has entry for: ${info.outputDir} — skipping duplicate.`);
  }
}
